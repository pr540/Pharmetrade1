                  ...
                  {cartItems.map((item, index) => (
                    <tr key={index}>
                      <td className="px-2 md:px-3 py-2 whitespace-nowrap">
                        <img
                          className="h-16 w-16 rounded-lg"
                          onClick={()=>handlemove(item.product.productID)}
                          src={item.product.imageUrl}
                          alt={item.product.id}
                        />
                      </td>
...
//     cartItemsFromRedux.map(item => ({ ...item, isUpdate: 0 }))
//   );  // const { cartItems, setCartItems } = useContext(AppContext);
//   const [quantities, setQuantities] = useState([]);
//   const navigate = useNavigate(); 
//   const carting = useSelector((state)=>state.cart.cart);
//   console.log(carting);
//   useEffect(() => {
//     // Initialize quantities based on the length of cartItems
//     setQuantities(cartItems.map(() => 1));
...
//       if (i === index) {
//         const quantityDifference = newQuantityNumber - item.quantity;
//         return {
//           ...item,
//           quantity: newQuantityNumber,
//           isUpdate: quantityDifference !== 0 ? quantityDifference : item.isUpdate
//         };
//       }
//       return item;
//     });
...
//                       <td className="px-2 md:px-4 py-3 whitespace-nowrap">{item.product.productName}</td>
//                       <td className="px-2 md:px-4 py-3 whitespace-nowrap">${item.product.salePrice}</td>
//                       <td className="px-2 md:px-4 flex py-3 whitespace-nowrap">
//                         <input
//                           type="number"
//                           value={item.quantity}
...
//                           className="text-xl border rounded-lg p-1 w-16"
//                         />
//                         <button onClick={()=>handleCart(item.product.productID,item.isUpdate)} className={`bg-blue-900 text-white px-3 ${item.isUpdate==0? "hidden":"flex"} `}>Update</button>
//                       </td>
//                       <td className="px-2 md:px-4 py-3 whitespace-nowrap">
//                         <strong>${calculateSubtotal(item.product.salePrice, quantities[index])}</strong>
//                       </td>
