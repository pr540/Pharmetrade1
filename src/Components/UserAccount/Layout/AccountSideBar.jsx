...
const AccountSideBar = () => {
  ...
  return (
    <div className={` p-4 absolute h-[calc(100%-80px)] bg-white font-normal overflow-scroll shadow-lg font-ubuntu ${isCollapsed ? 'min-w-16 items-center' : 'min-w-64'}`}>
      <div className="flex medium:hidden items-center justify-end p-2">
        <button onClick={toggleCollapse} className="text-gray-700 hover:text-blue-900">
      ...
      <nav className="space-y-2 text-[14px]">
        {links.map((link) => (
          <Link
            ...
            className={`flex items-center p-2 ${activeLink === link.path ? 'text-blue-900 bg-gray-200' : 'text-gray-700 hover:text-blue-900'} `}
          >
            <img src={link.icon} className="w-6 h-6" alt={link.name} />
